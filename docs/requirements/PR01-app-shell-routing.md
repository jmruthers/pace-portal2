...
- Current baseline behavior: `src/main.tsx` sets `APP_NAME`, runs `setupRBAC` with the base Supabase client before any RBAC-dependent route renders, mounts `QueryClientProvider`, `BrowserRouter`, pace-core **`ToastProvider`** (so `useToast` works under all routes), `UnifiedAuthProvider` (with inactivity + `InactivityWarningModal`, `SessionRestorationLoader`), and wraps the routed `App` in **`OrganisationServiceProvider`** so organisation context exists app-wide (see [Bootstrap provider order](#bootstrap-provider-order) below). `src/App.tsx` lazy-loads the portal pages; the authenticated shell is implemented as [`PortalAuthenticatedLayout`](../../src/shared/components/PortalAuthenticatedLayout.tsx) + [`ProfileCompleteLayout`](../../src/shared/components/ProfileCompleteLayout.tsx) (navigation-free), satisfying the [PaceAppLayout shell contract](../../docs/requirements/PR00-portal-architecture.md#paceapplayout-and-appswitcher) without necessarily importing the `PaceAppLayout` component by name. Public routes include `/login`, `/register`, and `/:eventSlug/:formSlug`; payment and invoice routes are not in the active rebuild.
...
Normative order for consuming apps (matches pace-core bootstrap checklist and avoids silent org-null bugs):

1. `QueryClientProvider`
2. `BrowserRouter`
3. **`ToastProvider`** (pace-core; required for `useToast` / global notifications)
4. `UnifiedAuthProvider` (session, user; receives `supabaseClient`)
5. **`OrganisationServiceProvider`** — **child of** `UnifiedAuthProvider`, with `supabaseClient`, `user`, and `session` from `useUnifiedAuthContext()` (often implemented as a small wrapper component around `App` inside `SessionRestorationLoader`).
6. Routed app (`App`).

**Do not omit `OrganisationServiceProvider`.** If it is missing, `useOrganisationsContextOptional()` returns `null`, `useUnifiedAuth()` falls back to stub organisation fields, and features that gate queries on `selectedOrganisation?.id` (profile completion wizard, org-scoped RPCs) will not load data. See [PR00 — Session, organisation context](./PR00-portal-architecture.md#session-organisation-context-and-proxy-product-behavior).
