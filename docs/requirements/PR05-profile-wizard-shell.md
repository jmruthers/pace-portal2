...
- **Implementation note (pace-portal):** `/profile-complete` uses `ProtectedRoute`, `ProfileCompleteLayout`, and `PagePermissionGuard` for `profile-complete` / `read` (see `src/constants/rbacPageNames.ts`). Requires PORTAL-DB-001 catalogue row and member `read` grants on the target Supabase project; deploy DB migrations before enabling fail-closed checks in production.
