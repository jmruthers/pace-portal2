...
- Treat proxy mode, auth handoff, and shared-schema compatibility as first-class architectural concerns.
...
- `AppSwitcher` from `@solvera/pace-core/components` appears **top left** in the authenticated header; populate items from apps the user can access. Omit on `/profile-complete` and auth entry routes.
...
- **Public files:** `usePublicFileDisplay` only for explicitly approved unauthenticated surfaces (for example future org-signup/lead exceptions). Authenticated surfaces use `FileDisplay` / `useFileDisplay` and `FileUpload` / `useFileUpload`.

### File display on event/member surfaces

- Event/member workflow surfaces in MVP are authenticated and should use `FileDisplay` or authenticated helpers (`useFileDisplay`), not bespoke storage URL generation.
...
- `/login`
- `/register`
- `/approvals/:token` (token approval host; see PR20)
- `/forms/:formSlug` (reserved for org-scoped form flows; may include public exceptions such as org signup when explicitly enabled)
...
**Matching rule:** Reserved paths such as `/login`, `/register`, `/dashboard`, `/profile-complete`, delegated profile routes, `/:eventSlug/application`, and `/:eventSlug/itinerary` must **not** be captured by the generic `/:eventSlug/:formSlug` matcher. The `applications` segment must not be captured as a form slug.

**Integrations (summary):** Supabase auth, tables, RPCs, storage; Google Maps Places loader; repo edge functions as present.

### Event workflow routes (product behavior)
...
### Cross-app: member-facing UI vs organiser apps (BASE, TEAM, …)
...
- **Out of MVP:** org lead/EOI and event lead/EOI.
- **Planned extension:** org signup uses dedicated workflow semantics beyond generic information collection; define in TEAM + portal follow-up slices, likely under `/forms/:formSlug`.
- **Token approval** (guardian/referee magic links; BASE **S07**) may stay a **thin standalone** page; **prefer pace-portal** as the host so participant-adjacent flows share one app, or keep a minimal BASE-origin page only if deployment constraints require it — see BASE architecture cross-cutting *Member-facing UI* (exceptions).
- **Operator/kiosk** flows (e.g. BASE scanning runtime) remain **out of portal**; they are not member self-service.
...
Reserved legacy slice IDs for a future payments wave: **POR-014, POR-015, POR-016**.
...
| POR-017 | [PR14-event-selector-and-hub.md](./PR14-event-selector-and-hub.md) | Event selector and hub |
...
| POR-020 | [PR17-form-journey-shell.md](./PR17-form-journey-shell.md) | Shared form journey shell |
...
- **PR03 vs PR14:** PR03 implements the current `EventList` placeholder; PR14 replaces it with **Apply / Resume / Manage** and owns `useFileReferences` for authenticated dashboard/event-hub logos.
