...
- implementation: dashboard slot `src/components/events/DashboardEventSelector.tsx`; selector + CTAs `src/components/events/EventList.tsx`; authenticated logos `src/components/events/EventLogo.tsx` + `src/hooks/events/useFileReferences.ts`; action mapping `src/hooks/events/eventDashboardAction.ts`; hub `src/pages/events/EventHubPage.tsx` + `src/hooks/events/useEventHub.ts`; routes `src/App.tsx` (event workflow routes and `/forms/:formSlug` nested under `PortalAuthenticatedLayout`); dashboard wiring `src/pages/DashboardPage.tsx`; landing/event visibility `src/shared/hooks/useEnhancedLanding.ts`, `src/shared/lib/dashboardEventVisibility.ts`. Participant hub uses interim `PagePermissionGuard` (`dashboard` read) plus row-level visibility in hub fetchers until dedicated RBAC catalogue rows exist.
